...
// debe checkear /showttos val, true: route a InventarioGusanos, false: accountLogged true
            ...
            <div className="create-account-modal" style={{marginTop:"70px"}}>
              ...
              {currentModal === 'show-create-acc-form' ? (
                <>
                  ...
                  <p className="my-1">Verifica tu identidad durante 24 horas en tu dispositivo validándote con tu correo electrónico.</p>
  ...
  const CreateAccountForm = () => {
    ...
    useEffect(() => {
      timeLeftToEnableAgain > 0 && setTimeout(() =>
        setTimeLeftToEnableAgain(timeLeftToEnableAgain -1), 1000);
        ...
        await api
          ...
          .then(function (response) {
            ...
            if (response.data.Success === 'emailsent') {
              toast.info('Código de confirmación enviada por correo. Por favor verifica tu inbox e introduce el código que enviamos.');
              setMailCodeSent(true)
              setTimeLeftToEnableAgain(60);
              setTimeout(()=>{setMailCodeSent(false)}, 60000)
            }
            })
    ...
    const VerifyMailCodeGame = async () => {
      await api
        .post('/verifymailcodegame',
          {params: {email: email, mailcode: mailcode}}
          )
        .then(function (response) {
    ...
    const ShowSetTOS = async () => {
      await api
        .post("/showtos", {params: {checktos: doesAccountHasShowTOSCheck}})
        .then(function (response){
    ...
    return (
      <form className="create-account-form mt-1" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex-wrapper my-1">
          ...
          <div className="ml-2">
            <button className="button sendcode" disabled={isMailCodeSent} onClick={createMailCodeGame}>
              {isMailCodeSent ? ("Enviar otro código(0:" + timeLeftToEnableAgain +")" ) : "Enviar código vía email"}
            </button>
          </div>
        </div>
          <div >
            <button className={"button"} type={'submit'} disabled={!isMailCodeSent} onClick={VerifyMailCodeGame}>Verificar Código</button>
          </div>
  ...
  return (
    <>
      {
        accountLogged
          ? !doesAccountHasShowTOSCheck ? <TerminosYCondicionesPage /> : console.log('navigate a pantalla de juego')
          : (<>
      ...
      <main className="market">
        <div className="hero">
          <div className="game-wrapper grid place-center p-1">
            ...
            <p className="mt-2">
              Conéctate con el correo con el que has comprado tu NFT para ser parte de la batalla espacial.
            </p>
            ...
            <p className="">
              ...
              confirmación. Ver términos de uso
            </p>

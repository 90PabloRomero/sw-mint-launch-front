...
export const GameHomePage = () => {
  ...
  useEffect(() => {
    const checkIfUserIsLoggedIn = async () => {
      await api.get('/api/checkuser', { params: { token: token } }).then(function (response) {
        if (response.data.Success === 'verified') {
          console.log('usuario verificado');
        } else {
          toast('Fallo de conexión. Por favor vuelva a iniciar sesión.');
          history('/');
        }
      });
    };
        ...
        <div className="footer-wrapper">
          ...
          > */}
              ...
              <Tooltip
                placement="bottom"
                trigger={['click']}
                overlay={<span>Misiones Diarias Proximamente!</span>}>
                <button className="blue ml-1" style={{ borderRadius: '16px' }}>
